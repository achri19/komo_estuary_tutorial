...
from use_orinoco import make_distance
...
def make_model_foundation(path,parameters,delta,folders,ref,distance,widths,watermask,pixel_step,build_path):
    ...
    # except:''
    if os.path.isfile("%s/%s_%s_%s.tif" %(elevationpath,delta,elev_name,xres))==False:
        ###########################################################################
        ...
        if landcovermethod == 'Copernicus':
            ...
            lut = np.genfromtxt(build_path + '/processing/configs/WorldCover_manningLUT.csv',delimiter = ',')
            np.savetxt('%s%s_%s_manningLUT.csv'%(folders[8],delta,landcovermethod),lut,delimiter=',')
        else:
        ...
        with rasterio.open("%s/%s_%s.tif" %(folders[4],elev_name,xres),'w', **save_profile) as dst:
            dst.write_band(1,elevation2.astype('float64'))
        with rasterio.open("%s/%s_landcover_final_%s.tif" %(folders[8],delta,xres),'w', **save_profile) as dst:
        ...
        #elevationpath  = folders[4] + elev_name
        elevation2 = rasterio.open('%s/%s_%s.tif' %(folders[3], elev_name,xres)).read(1)
