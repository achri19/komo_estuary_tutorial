...
def make_SWORD_networks(folders,ref,delta,parameters,pixel_step,watermaskname,skip=False):
    ...
    ##########################################################################
def make_segments_for_swot(folders,delta,ref,parameters,superpixel_area,skip):#,min_size,scale):
    step = 6
    print('\n##############################################################################################')
    ...
    if skip == False:
        ...
        oceanmask = np.where(ocean.read(1)==1,1,0)
        #oceanmask = np.where((ocean.read(1)==1)&(dem<0)&(dem>-100),True,False)

        dem = ref.read(1) #rasterio.open('%s%s_GEBCO_%s.tif' %(folders[7],delta,xres*3)).read(1)
        dem_masked = np.where(oceanmask==1,dem,0)
        ...
        ix = [0.0001,0.005,0.01]#compactness
        ...
        for i in range(0,len(ix)):
            for j in range(0,len(jx)):
                compactness = ix[i]
                nsegments = jx[j]
                if os.path.isfile('%s%s_oceandem_superpixels_%sm2_compact%s_segs%s_slic.shp' %(segments_dir,delta,superpixel_area,compactness,nsegments))==False:
                    ...
                    with rasterio.open(segments_dir+ '%s_oceandem_superpixels_%sm2_compact%s_segs%s_slic.tif' %(delta,superpixel_area,compactness,nsegments), 'w', **save_profile) as ds:
                        ds.write(superpixel_labels_s.astype(np.int32), 1)
                    try:os.remove('%s%s_oceandem_superpixels_%sm2_compact%s_segs%s_slic.shp'%(segments_dir,delta,superpixel_area,compactness,nsegments))
                    except:''
                    os.system('gdal_polygonize.py -q %s%s_oceandem_superpixels_%sm2_compact%s_segs%s_slic.tif '\
                                ...
                                %(segments_dir,delta,superpixel_area,compactness,nsegments,segments_dir,delta,superpixel_area,compactness,nsegments))
                else:
                    superpixel_labels_s = rasterio.open(segments_dir+ '%s_oceandem_superpixels_%sm2_compact%s_segs%s_slic.tif' %(delta,superpixel_area,compactness,nsegments)).read(1)
                axes[i,j].imshow(superpixel_labels_s,cmap=cmap)

        plt.tight_layout()
        plt.savefig('%s%s_Segments_Ocean_slic.png' %(segments_dir,delta))

        ix = [0.1,10,1000] #scale = Free parameter. Higher means larger clusters.
        ...
        for j in range(0,len(jx)):
            fig,axes = plt.subplots(ncols=len(zx),nrows=len(ix),figsize=(20,20))
            for i in range(0,len(ix)):
                for z in range(0,len(zx)):
                        scale = ix[i]
                        sigma = jx[j]
                        min_size = zx[z]
                        if os.path.isfile('%s%s_oceandem_superpixels_%sm2_scale%s_sigma%s_min%s_felz.shp' %(segments_dir,delta,superpixel_area,scale,sigma,min_size))==False:
                            ...
                            with rasterio.open(segments_dir + '%s_oceandem_superpixels_%sm2_scale%s_sigma%s_min%s_felz.tif' %(delta,superpixel_area,scale,sigma,min_size), 'w', **save_profile) as ds:
                                ds.write(superpixel_labels_f.astype(np.int32), 1)
                            try:
                                os.remove('%s%s_oceandem_superpixels_%sm2_scale%s_sigma%s_min%s_felz.shp' \
                                            %(segments_dir,delta,superpixel_area,scale,sigma,min_size))
                            except:''
                            os.system('gdal_polygonize.py %s%s_oceandem_superpixels_%sm2_scale%s_sigma%s_min%s_felz.tif '\
                                        ...
                                        %(segments_dir,delta,superpixel_area,scale,sigma,min_size,segments_dir,delta,superpixel_area,scale,sigma,min_size))
                        else:
                            superpixel_labels_f = rasterio.open(segments_dir + '%s_oceandem_superpixels_%sm2_scale%s_sigma%s_min%s_felz.tif' %(delta,superpixel_area,scale,sigma,min_size)).read(1)
                        axes[i,z].imshow(superpixel_labels_f,cmap=cmap)
...
def make_mesh_polygons(folders,delta,res,parameters,med_width,cellsperwidth,outdir,skip):
    ...
    if skip == False:
        ...
        #nearshore.geometry = nearshore.buffer(-10*xres)
        try: nearshore_no_holes=gpd.read_file("%s%s_nearshore_mesh_%s.shp" %(outdir,delta,xres))
        except:
            nearshore_no_holes, holes = delete_holes(nearshore)
