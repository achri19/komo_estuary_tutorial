...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 23 07:55:18 2020
@author: Alchrist
...
def get_extent_parameters(path,delta,folders,xres,parameters):
    ...
    np.savetxt('%s%s_extent.csv' %(folders[0],delta), extent,delimiter=',', fmt= '%1.3f') ## USED BY ANUGA MODEL
    ...
    try:
        ref_10m = rasterio.open('%s%s_GEBCO_%s.tif' %(folders[8],delta,ref_res))
    except:
        os.system('gdalwarp -overwrite -tr %s %s %sgebco_2020_geotiff/gebco_all.vrt %s%s_GEBCO_%s.tif '\
                  ...
                  %(ref_res,ref_res,input_path,folders[8],delta,ref_res,EPSG,ulx,lry,lrx,uly))
        ref_10m = rasterio.open('%s%s_GEBCO_%s.tif' %(folders[8],delta,ref_res))
...
def setup_AOI_files(path,delta,folders,xres,parameters):
    ...
    if os.path.isfile('%s%s_GEBCO_%s.tif' %(folders[8],delta,xres)) ==False:
    ...
    if os.path.isfile("%s%s_bathy_%s.tif" %(folders[8],delta,xres))==False:
         if oceanmethod == 'default' or oceanmethod == 'GEBCO':
             ...
             if os.path.isfile(folders[0] + oceanmethod + '.tif') == False:
                 print('File not found \n')
                 oceanmethod = input('What file would you like to use? Should be saved in /User_Defined_Files subfolder ')
             os.system('gdalwarp -overwrite -tr %s %s %s%s.tif %s%s_ocean.tif '\
             ...
             bathy = gdal.Open("%s%s_ocean.tif" %(folders[8],oceanmethod)).ReadAsArray()

         with rasterio.open("%s%s_bathy_%s.tif" %(folders[8],delta,xres),'w', **save_profile_xres) as dst:
             dst.write_band(1,bathy.astype('float64'))
         print('##################### Saving Ocean Bathymetry file as %s%s_bathy_%s.tif' %(folders[8].split(delta)[-1],delta,xres))
    ...
    if os.path.isfile("%s%s_topo_%s.tif" %(folders[8],delta + '_' +landmethod,xres))==False:
        if landmethod == 'default' or landmethod == 'NASADEM':
            print('##################### Downloading appropriate NASADEM tiles and merging into VRT')
            if not os.path.isfile('%s%s_NASADEM_topo_%s.tif' %(folders[8],delta,xres)):
                if not os.path.isfile('%s%s_NASADEM_egm08.tif' %(folders[8],delta)):
                    ...
                    os.system('gdalwarp -overwrite -srcnodata -9999 -dstnodata -9999 -wt Float64 -ot Float64 '\
                              '-t_srs "+proj=longlat +datum=WGS84 +no_defs +geoidgrids=%segm08_25.gtx" '\
                              '-s_srs "+proj=longlat +datum=WGS84 +no_defs +geoidgrids=%segm96_15.gtx" '\
                              '%s%s_NASADEM.vrt %s%s_NASADEM_egm08.tif -co COMPRESS=DEFLATE -q'
                ...
                os.system('gdalwarp -overwrite -tr %s %s -tap -te %s %s %s %s -srcnodata -9999 -dstnodata -9999 '\
                          '-s_srs EPSG:4326 -t_srs EPSG:%s %s%s_egm08.tif %s%s_topo_%s.tif -co COMPRESS=DEFLATE -q'
                          %(xres,xres,ulx,lry,lrx,uly,EPSG,folders[8],delta+'_'+landmethod,folders[8],delta+'_'+landmethod,xres))
        elif landmethod == 'GLO30':
            if not os.path.isfile('%s%s_topo_%s.tif' %(folders[8],delta+'_'+landmethod,xres)):
                if not os.path.isfile('%s%s_egm08.tif' %(folders[8],delta+'_'+landmethod)):
                    ...
                    os.system('gdalwarp -overwrite -srcnodata -9999 -dstnodata -9999 -wt Float64 -ot Float64 '\
                              '%s%s_GLO30.vrt %s%s_egm08.tif -co COMPRESS=DEFLATE -q'
            ...
            with rasterio.open("%s%s_topo_%s.tif" %(folders[8],delta + '_' + constantLandElev,xres),'w', **save_profile_xres) as dst:
            ...
            os.system('gdalwarp -overwrite -tr %s %s -tap %s%s%s.tif %s%s_%s_topo_%s.tif -t_srs EPSG:%s -te %s %s %s %s '\
        ...
        print('##################### Saving Land Topography file as %s%s_topo_%s.tif' %(folders[8].split(delta)[-1],delta,xres))
    ...
    if os.path.isfile('%s%s_%s_%s.tif' %(folders[8],delta,landcovermethod,xres))==False:
        if landcovermethod == 'default' or landcovermethod == 'WorldCover':
            print('##################### Landcover types based on Sentinel 1 and 2 World Cover Maps')
            if not os.path.isfile('%s%s_%s_%s.tif' %(folders[8],delta,landcovermethod,xres)):
                ...
                with open("%s%s_%s.txt" %(folders[1],delta,source), 'w') as f:
                    for item in landcovers:
                        f.write("%s\n" % item)
                # Merge NASADEM tiles to make topography file
                print('##################### Merging landcover tiles') #EPSG:4326++5733
                os.system("gdalbuildvrt %s%s_%s.vrt -input_file_list %s%s_%s.txt -a_srs EPSG:4326 "
                          %(folders[1],delta,landcovermethod,folders[1],delta,landcovermethod))
                os.system('gdalwarp -overwrite %s%s_%s.vrt %s%s_%s_%s.tif '\
            ...
            if not os.path.isfile('%s%s_%s_10.tif' %(folders[8],delta,landcovermethod)):
                ...
                os.system('gdalwarp -overwrite -tr %s %s %s %s%s_%s_%s.tif -t_srs EPSG:%s -te %s %s %s %s '\
    ...
    if os.path.isfile('%s%s_GMW_%s.tif' %(folders[8],delta,xres))==False:
        try:
            ...
            mangrove.to_file("%s%s_GMW.shp" %(folders[1],delta))     # Save
        except:
            print('There are no mangroves in the %s domain' %(delta))
            mangrove = np.where(ref_10m.read(1),0.,0.)
            with rasterio.open('%s%s_GMW_10.tif' %(folders[8],delta),'w', **save_profile_10m) as dst:
                dst.write_band(1,mangrove)
            mangrove = np.where(ref_array,0.,0.)
            with rasterio.open('%s%s_GMW_%s.tif' %(folders[8],delta,xres),'w', **save_profile_xres) as dst:
                dst.write_band(1,mangrove)
        else:
            print('Saving mangrove file as %s%s_GMW_10.tif' %(folders[8],delta))
            os.system('gdal_rasterize -burn 1 -tr %s %s -tap -te %s %s %s %s %s%s_GMW.shp %s%s_GMW_%s.tif '\
    ...
    if skip==False:
        ...
        if os.path.isfile('%s%s_10b.tif' %(folders[8],watermaskname))==False:
            with open("%sgee_ndwi_files.txt" %(folders[1]), 'w') as f:
                for item in gee:
                    f.write("%s\n" % item)
            os.system('gdalbuildvrt -input_file_list %s/gee_ndwi_files.txt -vrtnodata -9999 -srcnodata -9999 '\
        ...
        try: hydropoly = gpd.read_file("%s%s_hydropolys.shp" %(folders[0],delta))
        except:
        ...
        if clean_with_landcover:
            ...
            landcover = rasterio.open('%s%s_%s_10.tif' %(folders[8],delta,landcovermethod)).read(1).astype('int')
            mangrove = gdal.Open('%s%s_GMW_10.tif' %(folders[8],delta)).ReadAsArray()
            ...
            if os.path.isfile("%s%s_wetlands_%s.tif" %(folders[8],delta,xres))==False:
                if os.path.isfile("%s%s_wetlands_10.tif" %(folders[8],delta))==False:
                    ...
                    with rasterio.open("%s%s_wetlands_10.tif" %(folders[8],delta),'w', **save_profile_10m) as dst:
                        dst.write_band(1,iswetland)
                os.system('gdalwarp -overwrite -tr %s %s %s%s_wetlands_10.tif %s%s_wetlands_%s.tif -te %s %s %s %s '\
                          '-srcnodata -9999 -dstnodata -9999 -co COMPRESS=DEFLATE'
                          %(xres,yres,folders[8],delta,folders[8],delta,xres,ulx,lry,lrx,uly))
            iswetland = rasterio.open("%s%s_wetlands_10.tif" %(folders[8],delta)).read(1)

            print('\n[Step 3A][Make_Watermask][Delineating agriculture areas] ......\n')
            if os.path.isfile("%s%s_crops_%s.tif" %(folders[8],delta,xres))==False:
                if os.path.isfile("%s%s_crops_10.tif" %(folders[8],delta))==False:
                    iscrop = np.where(landcover==crop_class,1.,-9999)
                    iscrop = np.where(iscrop==1,1.,0.)
                    with rasterio.open("%s%s_crops_10.tif" %(folders[8],delta),'w', **save_profile_10m) as dst:
                        dst.write_band(1,iswetland)
                os.system('gdalwarp -overwrite -tr %s %s %s%s_crops_10.tif %s%s_crops_%s.tif -te %s %s %s %s -'\
                          'srcnodata -9999 -dstnodata -9999 -co COMPRESS=DEFLATE'
                          %(xres,yres,folders[8],delta,folders[8],delta,xres,ulx,lry,lrx,uly))
            iscrop = rasterio.open("%s%s_crops_10.tif" %(folders[8],delta)).read(1)
        ...
        watermaskvalue = int(zonal_stats(vectors = test_ocean['geometry'],raster = '%s%s_10.tif' %(folders[8],watermaskname),stats='majority',nodata = '-9999')[0]['majority'])
        ...
        if xres != 10:
            ...
            os.system('gdalwarp -overwrite -tr %s %s %s%s_expanded_10.tif %s%s_expanded_%s.tif -te %s %s %s %s -srcnodata -9999 '\
...
def more_opening(delta,folders,watermaskname,structure,ref,parameters):
    if structure == None:
        ...
        full_watermask_file = rasterio.open('%s%s_expanded_10.tif' %(folders[8],watermaskname))
        ...
        print('\n[Step 3A][Make_Watermask][Save as expanded water mask] ......\n')
        with rasterio.open('%s%s_expanded_10_%sx%s.tif' %(folders[8],watermaskname,structure,structure),'w', **save_profile_10m) as dst:
            dst.write_band(1,full_watermask)

        if xres != 10:
            print('\n[Step 3A][Make_Watermask][Resample to desired resolution %sm] ......\n' %(xres))
            os.system('gdalwarp -overwrite -tr %s %s %s%s_expanded_10_%sx%s.tif %s%s_expanded_%s_%sx%s.tif -te %s %s %s %s -srcnodata -9999 '\
                      '-dstnodata -9999 -co COMPRESS=DEFLATE'
                      %(xres,yres,folders[8],watermaskname,structure,structure,olders[8],watermaskname,xres,structure,structure,ulx,lry,lrx,uly))

        full_watermask = rasterio.open('%s%s_expanded_%s_%sx%s.tif' %(folders[8],watermaskname,xres,structure,structure))
        ...
        with rasterio.open('%s%s_watermask_%s.tif' %(folders[8],delta,xres),'w', **save_profile_xres) as dst:
            dst.write_band(1,full_watermask)
        ...
        with rasterio.open("%s%s_landmask_%s.tif" %(folders[8],delta,xres),'w', **save_profile_xres) as dst:
...
def make_polygons(delta,folders,parameters,ref,watermaskname,templates_path,skip=False):
    ...
    if skip == False:
        if os.path.isfile('%s%s_landmask_%sb.shp' %(folders[7],delta,xres))==False:
            ...
            replacements = {'start':  "$(conda shell.bash hook)",
                            ...
                            'command' : 'gdal_polygonize.py "%s%s_watermask_%s.tif" "%s%s_watermask_%s.shp" -8 -b 1 -f "ESRI Shapefile" %s DN' %(folders[8],delta,xres,folders[7],delta,xres,delta),
                            'command2' : 'gdal_polygonize.py "%s%s_landmask_%s.tif" "%s%s_landmask_%s.shp" -8 -b 1 -f "ESRI Shapefile" %s DN' %(folders[8],delta,xres,folders[7],delta,xres,delta)
                            }
        ...
        hydropoly = gpd.read_file("%s%s_hydropolys.shp" %(folders[0],delta))
        # hydropoly = gpd.overlay(hydropoly,extentpoly,how='intersection')
        hydrolake = gpd.read_file("%s%s_SWOTPLD.shp" %(folders[0],delta))
        #hydrolake = gpd.read_file("%s%s_HydroLAKES.shp" %(folders[0],delta))
        # hydrolake = gpd.overlay(hydrolake,extentpoly,how='intersection')

        ndwi_watermask = gpd.read_file("%s%s_watermask_%s.shp" %(folders[7],delta,xres))
        # ndwi_watermask = gpd.overlay(ndwi_watermask,extentpoly,how='intersection')
        ndwi_watermask['dissolve'] = 1

        ndwi_landmask = gpd.read_file("%s%s_landmask_%s.shp" %(folders[7],delta,xres))
        ...
        try:lakes = gpd.read_file("%s%s_lakes_%sb.shp" %(folders[7],delta,xres))
        except:
            try:
            ...
            if len(lakes) <1:
                ...
                print('##################### Lake polygons saved to %s%s_lakes_%s.shp' %(folders[7].split(delta)[-1],delta,xres))
        # else:
        ...
        try:oceans = gpd.read_file("%s%s_fulloceans_%sb.shp" %(folders[7],delta,xres))
        except:
        ...
        #     oceans = gpd.overlay(oceans,extentpoly,how='intersection')
        #     oceans.to_file("%s%s_fulloceans_%s.shp" %(folders[7],delta,xres))
        print('##################### Ocean polygons saved to %s%s_fulloceans_%s.shp' %(folders[7].split(delta)[-1],delta,xres))
        os.system('gdal_rasterize -burn 1 -tr %s %s -te %s %s %s %s %s%s_fulloceans_%s.shp %s%s_fulloceans_%s.tif -co COMPRESS=DEFLATE'  %(xres,yres,ulx,lry,lrx,uly,folders[7],delta,xres,folders[8],delta,xres))

        ## OCEANS AND LAKES together!
        print('\n[Step %s][Make_Polygons][Ocean and lakes] .......\n'%(step))
        try: ocean_lake = gpd.read_file("%s%s_ocean_and_lake_%sb.shp" %(folders[7],delta,xres))
        except:
            ...
            print('##################### Ocean and lake polygons saved to %s%s_ocean_lakes_%s.shp' %(folders[7],delta,xres))
        # else:
        ...
        try:
            water_connected_not_lakes = gpd.read_file("%s%s_water_connected_%sb.shp" %(folders[7],delta,xres))
        except:
            ...
            print('##################### All water connected to ocean polygons saved to %s%s_water_connected_%s.shp' %(folders[7].split(delta)[-1],delta,xres))
        # else:
        ...
        try:rivers = gpd.read_file("%s%s_rivers_%sb.shp" %(folders[7],delta,xres))
        except:
            ...
            print('##################### River polygons saved to %s%s_rivers_%s.shp' %(folders[7].split(delta)[-1],delta,xres))
        # else:
        #     rivers = gpd.overlay(rivers,extentpoly,how='intersection')
        #     rivers.to_file("%s%s_rivers_%s.shp" %(folders[7],delta,xres))
        #print('##################### River polygons saved to %s%s_rivers.shp' %(folders[7].split(delta)[-1],delta))
        os.system('gdal_rasterize -burn 1 -tr %s %s -te %s %s %s %s %s%s_rivers_%s.shp %s%s_rivers_%s.tif -co COMPRESS=DEFLATE'  %(xres,yres,ulx,lry,lrx,uly,folders[7],delta,xres,folders[8],delta,xres))

        ## CONNECTED WATER, ALL
        print('\n[Step %s][Make_Polygons][All water from connected water and lakes] .......\n'%(step))
        try: allwater = gpd.read_file("%s%s_water_connected_and_lakes_%sb.shp" %(folders[7],delta,xres))
        except:
        ...
        allwater.to_file('%s%s_water_connected_and_lakes_%s.shp' %(folders[7],delta,xres))
        print('##################### All water polygons saved to %s%s_water_connected_and_lakes_%s.shp' %(folders[7].split(delta)[-1],delta,xres))

        print('\n[Step %s][Make_Polygons][Land from area that is not water] .......\n'%(step))
        try:lands = gpd.read_file("%s%s_lands_%sb.shp" %(folders[7],delta,xres))
        except:
        ...
        lands.to_file("%s%s_lands_%s.shp" %(folders[7],delta,xres))
        print('##################### Land polygons saved to %s%s_lands_%s.shp' %(folders[7].split(delta)[-1],delta,xres))
        ...
        os.system('gdal_rasterize -burn 1 -tr %s %s -te %s %s %s %s %s%s_water_connected_and_lakes_%s.shp %s%s_water_connected_and_lakes_%s.tif -co COMPRESS=DEFLATE'  %(xres,yres,ulx,lry,lrx,uly,folders[7],delta,xres,folders[8],delta,xres))
        os.system('gdal_rasterize -burn 1 -tr %s %s -te %s %s %s %s %s%s_water_connected_and_lakes_%s.shp %s%s_water_connected_and_lakes_%s.tif -co COMPRESS=DEFLATE'  %(xres,yres,ulx,lry,lrx,uly,folders[7],delta,xres,folders[8],delta,xres))
        os.system('gdal_rasterize -burn 1 -tr %s %s -te %s %s %s %s %s%s_lands_%s.shp %s%s_lands_%s.tif -co COMPRESS=DEFLATE'  %(xres,yres,ulx,lry,lrx,uly,folders[7],delta,xres,folders[8],delta,xres))
        os.system('gdal_rasterize -burn 1 -tr %s %s -te %s %s %s %s %s%s_lakes_%s.shp %s%s_lakes_%s.tif -co COMPRESS=DEFLATE'  %(xres,yres,ulx,lry,lrx,uly,folders[7],delta,xres,folders[8],delta,xres))
...
def make_channel_networks(folders,delta,ref,parameters,pixel_step,skip=False):
    ...
    river = rasterio.open('%s%s_watermask_%s.tif' %(folders[8],delta,xres))
    ...
    if skip ==False:
        ...
        distance = make_distance(river,ocean,folders,delta,pixel_step,xres)
    else:
        print('\n[Step %s][Make_Channel_Networks] SKIP .......\n'%(step))

    distance = gdal.Open('%s%s_distance_%sx%s.tif' %(folders[8],delta,xres,pixel_step)).ReadAsArray()
    ...
    os.system('gdal_rasterize -burn 1 -ts %s %s -te %s %s %s %s '\
            '%s%s_river_centerline_%sx%s.shp %s%s_river_centerline_%sx%s.tif '\
            '-co COMPRESS=DEFLATE'
...
def make_model_foundation(path,parameters,delta,folders,ref,distance,widths,watermask,pixel_step,build_path):
    ...
    if os.path.isfile("%s%s_wetlands_%s.tif" %(folders[8],delta,xres)) == False:
        os.system('gdalwarp -overwrite -tr %s %s %s%s_wetlands_10.tif %s%s_wetlands_%s.tif -te %s %s %s %s '\
                  '-srcnodata -9999 -dstnodata -9999 -co COMPRESS=DEFLATE'
                  %(xres,yres,folders[8],delta,folders[8],delta,xres,ulx,lry,lrx,uly))
    iswetland = rasterio.open("%s%s_wetlands_%s.tif" %(folders[8],delta,xres)).read(1)
    ...
    # except:''
    if os.path.isfile("%s/%s_%s.tif" %(elevationpath,delta,elev_name))==False:
        ###########################################################################
        ...
        isocean = rasterio.open('%s%s_fulloceans_%s.tif'  %(folders[8],delta,xres)).read(1)
        ...
        isriver = rasterio.open('%s%s_rivers_%s.tif' %(folders[8],delta,xres)).read(1)
        isriver = np.where(isriver ==1,1.,0.)
        try:
            islake = rasterio.open('%s%s_lakes_%s.tif'  %(folders[8],delta,xres)).read(1)
            islake = np.where(islake==1,1.,0.)
        except: islake = np.where(ref_array,0,0)
        island = rasterio.open('%s%s_lands_%s.tif'  %(folders[8],delta,xres)).read(1)
        ...
        mangrove = gdal.Open('%s%s_GMW_%s.tif' %(folders[8],delta,xres)).ReadAsArray()
        if landcovermethod == 'Copernicus':
            mangrove_class = 9
        elif landcovermethod == 'WorldCover':
            mangrove_class = 95

        landcover = rasterio.open('%s%s_%s_%s.tif' %(folders[8],delta,landcovermethod,xres)).read(1).astype('int')
        landcover[landcover==-9999] = 0
        ...
        with rasterio.open("%s%s_water_type.tif" %(folders[8],delta),'w', **save_profile) as dst:
        ...
        if landcovermethod == 'Copernicus':
            ...
            np.savetxt('%s%s_%s_manningLUT.csv'%(folders[8],delta,landcovermethod),lut,delimiter=',')
        elif landcovermethod == 'WorldCover':
        ...
        with rasterio.open("%s%s_%s_%s.tif" %(folders[8],delta,frict_name,landcovermethod),'w', **save_profile) as dst:
            dst.write_band(1,manning.astype('float64'))

        ##########################################################################
        ########################## River Centerline ##############################
        ##########################################################################
        centerline = gdal.Open('%s%s_river_centerline_%sx%s.tif'  %(folders[8],delta,xres,pixel_step))
        drv = gdal.GetDriverByName('GTiff')
        dst_ds = drv.Create('%s%s_river_proximity_%sx%s.tif'  %(folders[8],delta,xres,pixel_step),
        ...
        src_ds = None
        dst_ds = None
        proximity = gdal.Open('%s%s_river_proximity_%sx%s.tif'  %(folders[8],delta,xres,pixel_step)).ReadAsArray()# * (widths>0)
        ...
        prox_widths = segments.join(pd.DataFrame(zonal_stats(segments, '%s%s_river_proximity_%sx%s.tif'  %(folders[8],delta,xres,pixel_step),stats="max")))
        prox_widths['newwidth'] = prox_widths['max']*2
        prox_widths = prox_widths[prox_widths['DN']!=0]
        try:
            os.remove('%s%s_widthsfromproximity_%sx%s.shp' %(folders[7],delta,xres,pixel_step))
        except: ''
        prox_widths.to_file('%s%s_widthsfromproximity_%sx%s.shp' %(folders[7],delta,xres,pixel_step), driver='ESRI Shapefile')
        os.system('gdal_rasterize -a newwidth -ts %s %s -te %s %s %s %s '\
                  '%s%s_widthsfromproximity_%sx%s.shp %s%s_widthsfromproximity_%sx%s.tif -co COMPRESS=DEFLATE'
                  %(distance.shape[1],distance.shape[0],ulx,lry,lrx,uly,folders[7],delta,xres,pixel_step,folders[8],delta,xres,pixel_step))
        widths = rasterio.open('%s%s_widthsfromproximity_%sx%s.tif' %(folders[8],delta,xres,pixel_step)).read(1)
        ...
        bathy = rasterio.open("%s%s_bathy_%s.tif" %(folders[8],delta,xres)).read(1)
        ...
        if upper_rivermethod == 'default' or upper_rivermethod == 'GEBCO':
            ...
            while True:
                try:
                    ...
                    os.system('gdalwarp -overwrite -tr %s %s -tap %s%s.tif %s%s_upperriver.tif -t_srs EPSG:%s -te %s %s %s %s -srcnodata -9999 -dstnodata -9999'  %(xres, yres,folders[0],upper_rivermethod,folders[1],upper_rivermethod,EPSG,ulx,lry,lrx,uly))
                ...
                riverbathy = gdal.Open("%s%s_upperriver.tif" %(folders[2],upper_rivermethod)).ReadAsArray()
        ...
        if lower_rivermethod == 'default' or lower_rivermethod == 'GEBCO':
            ...
            while True:
                try:
                    ...
                    os.system('gdalwarp -overwrite -tr %s %s -tap %s%s.tif %s%s_lowerriver.tif -t_srs EPSG:%s -te %s %s %s %s -srcnodata -9999 -dstnodata -9999 co COMPRESS=DEFLATE'  %(xres, yres,folders[0],lower_rivermethod,folders[1],lower_rivermethod,EPSG,ulx,lry,lrx,uly))
                except AttributeError:
                    print('File not found \n')
                ...
                riverbathy = gdal.Open("%s%s_lowerriver.tif" %(folders[1],lower_rivermethod)).ReadAsArray()
        ...
        with rasterio.open("%s/%s_temp.tif" %(folders[1],delta),'w', **save_profile) as dst:
        ...
        topo = rasterio.open("%s%s_topo_%s.tif" %(folders[8],delta+'_'+landmethod,xres)).read(1)
        ...
        if lakemethod == 'default' or lakemethod == 'GEBCO':
            ...
            while True:
                try:
                    ...
                    os.system('gdalwarp -overwrite -tr %s %s -tap %s%s%s.tif %s%s%s_lake.tif -t_srs EPSG:%s -te %s %s %s %s -srcnodata -9999 -dstnodata -9999 co COMPRESS=DEFLATE'  %(xres, yres,deltapath,folders[3],lakemethod,deltapath,folders[2],lakemethod,EPSG,ulx,lry,lrx,uly))
                    lake_elev = gdal.Open("%s_land.tif" %(lakemethod)).ReadAsArray()
                except AttributeError:
                    print('File not found \n')
            ...
            print('##################### Wetland elevation set with:  %sm uniform' %(constantWetlandElev))
             ...
             while True:
                try:
                    ...
                    os.system('gdalwarp -overwrite -tr %s %s -tap %s%s%s.tif %s%s%s_wetland.tif -t_srs EPSG:%s -te %s %s %s %s -srcnodata -9999 -dstnodata -9999 co COMPRESS=DEFLATE'  %(xres, yres,deltapath,folders[3],wetlandmethod,deltapath,folders[2],wetlandmethod,EPSG,ulx,lry,lrx,uly))
                    wetland_elev = gdal.Open("%s%s_wetland.tif" %(folders[0],wetlandmethod)).ReadAsArray()
                except AttributeError:
                    print('File not found \n')
        ...
        with rasterio.open("%s/%s_%s.tif" %(elevationpath,delta,elev_name),'w', **save_profile) as dst:
            dst.write_band(1,elevation2.astype('float64'))
        with rasterio.open("%s/%s_landcover_final.tif" %(folders[8],delta),'w', **save_profile) as dst:
        ...
        plt.savefig("%ssummary_figures/bathy/%s_bathy.png" %(path,delta))
        ...
        plt.savefig("%ssummary_figures/watermasks/%s_riverlandocean.png" %(path,delta))
        ...
        elevationpath  = folders[4] + elev_name
        elevation2 = rasterio.open('%s/%s_%s.tif' %(elevationpath, delta, elev_name)).read(1)
        ...
        plt.tight_layout()
        plt.savefig("%s%s_%s.png" %(folders[9],delta,elev_name))
        plt.savefig("%ssummary_figures/DEM/%s_%s.png" %(path,delta,elev_name))

    return elevation2, elev_name,elevationpath
...
def set_boundary_conditions(delta,folders,res,parameters):
    ...
    intersections['mean'] = pd.DataFrame(zonal_stats(vectors = intersections['geometry'],raster = "%s%s_bathy_%s.tif" %(folders[8],delta,xres),stats='mean',nodata = '-9999'))['mean']
...
def get_inlet(delta,folders,res,parameters,inletx,inlety):
    step = '8b'
    print('\n\n\n##############################################################################################')
...
def get_tidal_boundary(delta,folders,res,parameters,tide_bnd,tidey,tidex):
    ...
    if len(tide_bnd) > 0:
        ...
        print('\n[Step %s][Set_Boundary_Conditions][Deepest model side = downstream boundary, tidal conditions] .......\n'%(step))
        # tide = df_line.loc[df_line['mean']==min(df_line['mean'])]
        df_line['boundary'][df_line['mean']==min(df_line['mean'])] = 'Tide'
        #deepest = int(df_line['id'][df_line['mean']==min(df_line['mean'])])
    if (tidex == -9999) & (tidey == -9999) :
        tide_lines = df_line[df_line['boundary']=='Tide'].geometry
        #tide_lines = tide_lines.to_crs('EPSG:4326')

        tidex = tide_lines[tide_lines.length == max(tide_lines.length)].centroid.iloc[0].coords.xy[0][0]
        tidey = tide_lines[tide_lines.length == max(tide_lines.length)].centroid.iloc[0].coords.xy[1][0]
